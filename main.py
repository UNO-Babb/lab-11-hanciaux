...
def shopper(env, id):
    ...
    shoppingTime = items // (random.randint(1, 3)) # shopping takes 1/2 a minute per item.
...
def checker(env):
    ...
    while True:
        ...
        items = customer[1]
        checkoutTime = items // (random.randint(8,12)) + 1
...
def customerArrival(env):
    ...
    while True:
        ...
        yield env.timeout(random.randint(0, 5)) #New shopper every two minutes
